...
% b = 1 - n^2 * a;
% m_lb = 1 - n^4 ./ ( 1./b .* log(1 + b ./ a) );

D = (n^2 - 1) ./ (exp(lambda) + n^2 - 1);


i_c = 1 ./ (exp(lambda) + n^2 - 1);
j_c = (exp(lambda) - 1) .* i_c;
...
for L = 1:length(lambda)
    ...
    % Compute mtx A
    H1_diag = ones(n^2) / a;
    H1_diag(D_max, :) = (log(a+b) - log(a)) / b; H1_diag(:, D_max) = (log(a+b) - log(a)) / b;
    H1_diag(D_max, D_max) = 1 / (a+b);
    ...
    [i_psi, j_psi] = find(beta);
    i_psi = [(1:n^2)'; i_psi]; j_psi = [(1:n^2)'; j_psi];
    
    U_sub = sparse((i_psi - 1)*n^2 + j_psi, 1:length(i_psi), 1);
    P_sub = U_sub * U_sub';
    
    H2 = sparse(idx, jdx, n);

    idx_mtx = zeros(2*n^2-n, n);
    for i = 1:n
        idx_mtx(i:n:n^2, i) = 1;
    end
    [U_I, D] = eig(idx_mtx*idx_mtx');
    U_I = sparse(U_I);
    U_I = U_I(:, 1:end-n);
%     I = ones(n^2, 1);
%     [U_I, ~] = eig(I*I');
%     U_I = [ U_I(:, 1:end-1), zeros(n^2, n^2-n); zeros(n^2-n, n^2-1), eye(n^2-n) ];
%     U_I = sparse(U_I);
    
    P_I = (U_I * U_I');
    
    H1u = full(U_I'*U_sub'*H1*U_sub*U_I);
    H2u = full(U_I'*U_sub'*H2*U_sub*U_I);
