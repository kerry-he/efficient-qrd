...
for k = 1:M
    ...
    ub_std(k) = max(grad);
    if k > 2
        ub_std(k) = min(ub_std(k), ub_std(k-1));
    end
...
%     end    
    ...
    ub_heur(k) = max(grad);
    if k > 2
        ub_heur(k) = min(ub_heur(k), ub_heur(k-1));
    end
...
semilogy(min([ub_std, ub_heur, ub_ls]) - obj_std)
hold on
semilogy(min([ub_std, ub_heur, ub_ls]) - obj_heur)
semilogy(min([ub_std, ub_heur, ub_ls]) - obj_ls)
