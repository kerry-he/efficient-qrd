...
RHO_Q = PartialTrace(RHO_QR, 2, N);
obj(1) = S_A + von_neumann_entropy(PartialTrace(RHO_QR)) - von_neumann_entropy(RHO_QR); 
...
while true
    ...
    while true
        ...
        LOG_RHO_QR = logm(RHO_QR_prev);
        GRAD_RHO = LOG_RHO_QR - kron(logm(PartialTrace(RHO_QR_prev, 2, N)), eye(N));
        STEP = -GRAD_RHO + lambda_bar*AR - kron(speye(N), V_bar);
        
        RHO_QR = expm(LOG_RHO_QR + STEP*t_p);
        RHO_QR = RHO_QR / trace(RHO_QR);
        
        V_CV = V_prev + (PartialTrace(RHO_QR, 1, N) - R) * t_d;
        lambda = lambda_prev + t_d * (1 - (AR_vec' * RHO_QR * AR_vec) - DIST)*0;

        obj(k) = S_A + von_neumann_entropy(PartialTrace(RHO_QR)) - von_neumann_entropy(RHO_QR); 

        if obj(k) - obj(k - 1) - trace(GRAD_RHO * (RHO_QR - RHO_QR_prev)) ...
            + trace((V_CV - V_bar) * PartialTrace(RHO_QR - RHO_QR_prev, 1, N)) - (lambda-lambda_bar)*(AR_vec' * (RHO_QR - RHO_QR_prev) * AR_vec) ...
                <= abs(quantum_relative_entropy(RHO_QR, RHO_QR_prev) - trace(RHO_QR) + trace(RHO_QR_prev)) / t_p...
...
%     p_res(k) = (quantum_relative_entropy(RHO_QR, RHO_QR_prev) - trace(RHO_QR) + trace(RHO_QR_prev)) / (t_p);
% 
...
%     minimize (quantum_entr(A) - quantum_cond_entr(rho, [N, N], 2));
%     
%     rho >= 0;
%     PartialTrace(rho, 1, N) == R;
%     1 - AR_vec' * rho * AR_vec <= DIST
% cvx_end
% toc
% 
% semilogy(cumsum(time(1:end-1)), abs(obj - cvx_optval))
