...
for k = 1:K
    ...
    obj_cv(k) = S_A + von_neumann_entropy(RHO_Q); 
    obj_cv(k) = obj_cv(k) - shannon_entropy(RHO_QR_D);
    lgn_cv(k) = obj_cv(k) - lambda*(AR_vec' * RHO_QR * AR_vec);

    % Perform Blahut-Arimoto iteration
    [RHO_QR, RHO_QR_D, RHO_QR_U, V_CV] = trace_preserve_V(RHO_QR, V_CV, AR_vec, lambda);

    % Compute upper bound
    grad = kron((log(diag(RHO_Q)) - log(diag(PartialTrace(RHO_QR, 2, N)))), ones(N, 1));
...
function [RHO_QR, rho_D, rho_U, V] = trace_preserve_V(RHO_QR, V, AR, lambda)
    NR = length(V);
    N = length(RHO_QR) / NR;
...
function out = diagPartialTrace(RHO, sys, N)
    ...
    if sys == 2
        for i = 1:N:N^2
            out(i) = sum(RHO(i:i+N-1, i:i+N-1), 'all');
        end
    else
        for i = 1:N
